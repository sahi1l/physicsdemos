console.log("Defining Refraction now!")
var name,W=400,H=400;
var paper;
//Object definitions
var Index=function(x,y,color,n0){
    ...
    this.set=function(nval){
        ...
        DrawOtherRays();
	//            this.label.attr("text",(" "+(this.val)).slice(-2));
	//            this.hilabel.attr("text",(this.val<max)?(" "+(this.val+1)).slice(-2):"");
	//            this.lolabel.attr("text",(this.val>min)?(" "+(this.val-1)).slice(-2):"");            
...
DrawOtherRays=function(ang){
    ...
    else {refracted.set(Math.abs(Math.asin(Sb)));}
...
init=function(_name,_W,_H){
    name=_name;W=_W;H=_H;
    console.log("Defining paper: ",name,W,H)
    ...
    console.log("Now into the objects")
