...
var Diffusion=new function(){
	...
	first:1,
		...
		this.ylabels()
            ...
            this.left=paper.path("").attr({stroke:"#4a4","stroke-width":2});
            this.right=paper.path("").attr({stroke:"#f4f","stroke-width":2});
        },
	xlabels:function(){
	    for(let i=1;i<=10;i++){
	...
	ylabels:function(){
	    for(let i=1;i<=10;i++){
		Graph.lbly[i-1].attr("text",20+2*parseFloat(i)+"°C")
	    }
	},
        draw:function(n){
...
//            console.log(this.data[0]+","+p);
	...
	var cnt=0;
		...
		cnt+=1;
            ...
            if(twoQ){
                ...
                if(graphQ){
		    hot+=2*(wwindow.walker2[w].x<Nx/2);
		    cnt+=1;
		}
    ...
    this.init=function(_name,_W,_H){
        ...
        wwindow.padx=5;
        wwindow.borderL=paper.rect(0,0,0,0).attr({fill:"#8f8"});
        wwindow.borderR=paper.rect(0,0,0,0).attr({fill:"#f8f"});
	var that=this;
