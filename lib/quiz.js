...
export class Canvas {
    constructor($root,W,H) {
        this.W = W??300;
        this.H = H??300;
        this.$w = $("<div>").addClass("canvas"); //.attr({width:this.W, height:this.H});
        if ($root) {
            this.$w.appendTo($root);
            this.$root = $root;
        }
        this.paper = Raphael(this.$w[0],this.W,this.H);
        this.paper.path("M0,0L100,100").attr("stroke-width",4);
    }
    clear() {
        //might want to clear out the svg code instead?
        console.debug("Clearing canvas");
        this.paper.clear();
    }
}
export class Score {
    constructor($root, maxtries, questiongenerator, settings) {
        //questiongenerator is a function that returns an object
        ...
        if (this.settings.canvas) {
            this.canvas = new Canvas($root, 300, 300);
//            this.canvas.$w.appendTo($root); //this is a default at least, could be overridden
        }
        ...
        this.$left = $("<span>").addClass("scoreleft")
            .appendTo(this.$w);
        ...
        this.$right = $("<span>").addClass("scoreright")
            .appendTo(this.$w);
        this.$correct = $("<span>").html("0")
            .addClass("scoreheader right");
        this.$errors = $("<span>").html("0")
            .addClass("scoreheader wrong");
...
//            this.$reaction.html("No, sorry.").addClass("wrong");
        ...
        if (this.guessesLeft==0 || correctQ){
            this.complete = true;
            $("demo-quiz > button").addClass("disabled");
...
//	if(MathJax) {MathJax.typeset();}
    ...
    render() {
        ...
        if (this.prams.sort.startsWith("n")){
            ...
            shf = shf.sort((a,b)=>{return this.answers[a]>this.answers[b] ? 1 : (this.answers[a]<this.answers[b] ? -1 : 0)});
        } else {
            shf = shuffle(shf);
        }
        for (let a of shf) {
